...
def listar_solicitudes_xlsx() -> List[Path]:
    from datetime import datetime, date

    archivos = [p for p in SOL_DIR.glob("*.xlsx") if not p.stem.endswith("-B")]

    def _fecha(p: Path) -> date:
    ...
    return sorted(archivos, key=_fecha, reverse=True)
